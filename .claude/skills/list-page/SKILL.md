...
**3. Reuse what is shared.** `HistoryModal`, `PageSizeSelect`, `LocalDate` and
everything in `src/lib/updateHistory.ts` are shared across all four pages.
Never fork them. Only the page, the two endpoints and the entity's own modals
are new.
